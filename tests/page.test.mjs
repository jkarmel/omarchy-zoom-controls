...
try {
 ...
 c=await CDP.connect(ts.find(t=>t.type==='page').webSocketDebuggerUrl);
...
}finally{c?.close();const exited=once(browser,'exit');browser.kill();await exited;await rm(tmp,{recursive:true,force:true})}
